  ...
  if (document.getElementById){
    ...
    for (i=0; i<(args.length-2); i+=3) { test=args[i+2]; val=document.getElementById(args[i]);
      if (val) { nm=val.name; if ((val=val.value)!="") {
        if (test.indexOf('isEmail')!=-1) { p=val.indexOf('@posta.pau.edu.tr');
          if (p<1 || p==(val.length-1)) errors+='- '+nm+' must contain an Pamukkale University student e-mail address.\n';
...
<option value="AUG">AGUSTOS</option>
<option value="SEP">EYLUL</option>
<option value="OCT">EKIM</option>
<option value="NOV">KASIM</option>
